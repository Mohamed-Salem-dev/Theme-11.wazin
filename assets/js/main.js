...
$(document).ready(function() {
  ...
  $('.btn-add-Products').on('click', function() {
      ...
      const productTitle = productItem.find('.titel-item-product').text();
      const productPrice = parseFloat(productItem.find('.price-discount').text());
          ...
          $('.box-item-product-aside-cart').append(`
              <li class="item-product-aside-cart" data-product-id="${productId}">
                  ...
                  <div class="text-end-isp">
                      ...
                      <div>
                          <span class="price-item-aside">${productPrice.toFixed(2)} </span><span>S.R</span>
                      </div>
                  </div>
              </li>
          `);
  ...
  $(document).on('click', '.btn-delete-product', function() {
      ...
      $(`.item-product-js[data-product-item=${productId}] .btn-add-Products`).removeClass('active');
      updateCartCount();
      updateTotal();
  ...
  function updateCartCount() {
      const itemCount = Object.keys(cartItems).reduce((count, id) => count + cartItems[id].quantity, 0);
  ...
  function updateItemTotal(productId) {
      ...
      $(`.item-product-aside-cart[data-product-id=${productId}] .price-item-aside`).text(total.toFixed(2));
  }

  function updateTotal() {
      const total = Object.keys(cartItems).reduce((sum, id) => sum + cartItems[id].price * cartItems[id].quantity, 0);
...
$(document).ready(function() {
  ...
  $('.btn-Favorite-product').on('click', function() {
      const productItem = $(this).closest('.item-product-js');
      const productId = productItem.data('product-item');
      const productTitle = productItem.find('.titel-item-product').text();
      const productPrice = parseFloat(productItem.find('.price-discount').text());
          ...
          $('.box-item-product-aside-Favorite').append(`
              <li class="item-product-aside-Favorite" data-product-id="${productId}">
                  <div class="d-flex gap-2 align-items-center">
                      ...
                      <div>
                          <a href="#"><h3>${productTitle}</h3></a>
                          <span class="price-item-aside-Favorite">${productPrice.toFixed(2)} </span><span>S.R</span>
                      </div>
                  </div>
                  <div class="text-end-isp">
                      <button class="btn btn-delete-product"><i class="fa-solid fa-trash"></i></button>
  ...
  $(document).on('click', '.btn-delete-product', function() {
      ...
      $(`.item-product-js[data-product-item=${productId}] .btn-Favorite-product`).removeClass('active');
      updateFavoriteCount();
  });
